...
import { Link2, Plus, X, Building2, Briefcase, FileText, BookOpen } from "lucide-react";
...
export function LinkedEntities({ pageId, canEdit }: { pageId: string; canEdit: boolean }) {
  ...
  const { data: wikiPages = [] } = useQuery({
    queryKey: ["wiki-resolve-pages"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("wiki_pages")
        .select("id, title, slug, wiki_spaces(slug)")
        .neq("id", pageId);
      if (error) throw error;
      return (data ?? []).map((p) => {
        const spaceObj = Array.isArray(p.wiki_spaces) ? p.wiki_spaces[0] : p.wiki_spaces;
        return {
          id: p.id,
          title: p.title,
          slug: p.slug,
          spaceSlug: spaceObj?.slug,
        };
      }) as EntityLite[];
    },
  });
    ...
    <div className="border border-border rounded-lg p-5 bg-paper">
      ...
      {links.length === 0 ? (
        ...
        <ul className="flex flex-wrap gap-2">
          {links.map((l) => {
            const Icon = ICON[l.entity_type] || BookOpen;
            ...
            const pageObj =
              l.entity_type === "wiki_page" ? wikiPages.find((p) => p.id === l.entity_id) : null;

            return (
              <li key={l.id}>
                <div className={cls}>
                  {l.entity_type === "wiki_page" && pageObj?.spaceSlug && pageObj?.slug ? (
                    <Link
                      to="/wiki/$spaceSlug/$pageSlug"
                      params={{ spaceSlug: pageObj.spaceSlug, pageSlug: pageObj.slug }}
...
function AttachEntityPopover({
  ...
  const [tab, setTab] = useState<EntityType>("wiki_page");
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-80 p-0">
        <div className="flex border-b border-border">
          {(["wiki_page", "client", "project", "prompt"] as EntityType[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex-1 px-2 py-2 text-[10px] font-mono uppercase tracking-wider transition ${
                tab === t
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((r) => (
              <button
                key={r.id}
                onClick={() => attach(r.id)}
                className="w-full text-left px-3 py-2 text-sm hover:bg-paper-soft transition truncate flex items-center justify-between"
              >
