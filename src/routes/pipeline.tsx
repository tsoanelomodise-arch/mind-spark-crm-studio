...
import { GripVertical, Briefcase, Plus, Repeat, Archive, ArchiveRestore, ChevronDown, ChevronRight, CalendarClock, CalendarCheck2, CalendarDays, AlertTriangle, CalendarPlus, Eye, EyeOff, Coins, Users, XCircle, RotateCcw } from "lucide-react";
...
function PipelinePage() {
  ...
  const { data: allClientNotes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("client_notes").select("id, project_id, client_id");
      if (error) return [];
      return data ?? [];
    },
  });
  ...
  const { data: projects = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("projects")
        .select("id,name,status,impl_stage,notes,client_id,updated_at,repeat_interval,archived_at,start_date,due_date,delivered_at,next_occurrence_date,opportunity_value,project_type")
  ...
  const boardProjects = useMemo(
    () =>
      filteredActive.filter((p) => {
        if (!showDelivered && p.status === "delivered") return false;
        if (!showLost && p.status === "lost") return false;
        return true;
      }),
    [filteredActive, showDelivered, showLost],
  );
  ...
  const lostCount = useMemo(
    () => filteredActive.filter((p) => p.status === "lost").length,
    [filteredActive],
  );
  ...
  const displayedStages = useMemo(
    () =>
      PIPELINE_STAGES.filter((s) => {
        if (s.id === "delivered" && !showDelivered) return false;
        if (s.id === "lost" && !showLost) return false;
        return true;
      }),
    [showDelivered, showLost],
  );
  ...
  const move = async (projectId: string, stage: PipelineStage) => {
    ...
    const updates: { status: PipelineStage; delivered_at?: string | null; impl_stage?: string | null } = { status: stage };
    if (stage === "work_in_progress" && !current.impl_stage) updates.impl_stage = "kickoff";
    if (stage === "delivered" && !current.delivered_at) {
      updates.delivered_at = nowIso;
      if (!current.impl_stage) updates.impl_stage = "done";
    }
    ...
    <div className="mx-auto max-w-[1400px] px-6 py-10">
      <div className="flex flex-wrap items-end justify-between gap-6 mb-8 pb-8 border-b border-border">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.2em] font-semibold text-muted-foreground">
            ...
            {overdueCount > 0 && (
              <> · <span className="text-destructive font-bold">{overdueCount} overdue</span></>
            )}
          </p>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
        ...
        <button
          ...
          title={showLost ? "Hide lost projects from the board" : "Show lost projects on the board"}
        >
          {showLost ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}
          Lost
          <span className={`tabular-nums ${showLost ? "opacity-80" : "opacity-60"}`}>
            {lostCount}
          </span>
        </button>
      </div>
        ...
        <>
          ...
          <div
            className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ${
              displayedStages.length === 6
                ? "xl:grid-cols-6"
                : displayedStages.length === 5
                ? "xl:grid-cols-5"
                : displayedStages.length === 4
                ? "xl:grid-cols-4"
                : "xl:grid-cols-3"
            }`}
          >
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-1.5 flex flex-wrap gap-1">
                                ...
                                <button
                                  ...
                                  className={`inline-flex items-center gap-1 font-mono text-[10px] uppercase tracking-widest border rounded-full px-1.5 py-0.5 cursor-pointer transition ${
                                    hasSavedQuoteRecord(p.id)
                                      ? "border-primary/40 bg-primary/10 text-primary font-semibold hover:bg-primary/20"
                                      : "border-border bg-paper-soft hover:border-primary hover:text-primary"
                                  }`}
                                  title={hasSavedQuoteRecord(p.id) ? "Revisit & Edit Saved Quote" : "AI Quote & Price Estimator"}
                                >
                                  <Calculator className="h-2.5 w-2.5 text-primary" />
                                  {hasSavedQuoteRecord(p.id) ? "Edit Quote" : "Quote"}
                                </button>
                                <button
                                  type="button"
                                  onClick={(e) => {
                                  ...
                                  className={`inline-flex items-center gap-1 font-mono text-[10px] uppercase tracking-widest border rounded-full px-1.5 py-0.5 cursor-pointer transition ${
                                    (p.notes?.trim() || (projectNotesCount.get(p.id) ?? 0) > 0)
                                      ? "border-sky-500/40 bg-sky-500/10 text-sky-700 dark:text-sky-300 font-medium hover:bg-sky-500/20"
                                      : "border-border bg-paper-soft text-muted-foreground hover:border-sky-500 hover:text-sky-600"
                                  }`}
                                  title="Access & Edit Project Notes"
                                >
                                  <FileText className="h-2.5 w-2.5 text-sky-600 dark:text-sky-400" />
                                  Notes
                                  {((projectNotesCount.get(p.id) ?? 0) > 0 || p.notes?.trim()) && (
                                    <span className="font-semibold">
                                      {(projectNotesCount.get(p.id) ?? 0) > 0 ? `(${projectNotesCount.get(p.id)})` : ""}
                                    </span>
                                  )}
                                </button>
                              ...
                              {p.notes && (
                                <button
                                  ...
                                  className="mt-1.5 text-left text-[11px] text-muted-foreground hover:text-foreground line-clamp-2 cursor-pointer transition block w-full hover:underline"
                            ...
                            <div className="flex flex-col items-center gap-1 shrink-0">
                              <button
                                ...
                                title="Project Notes"
                                aria-label="Access project notes"
                              >
                              ...
                              {p.status !== "lost" ? (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    move(p.id, "lost");
                                    toast.info(`"${p.name}" moved to Lost stage`);
                                  }}
                                  ...
                                  aria-label="Mark project as Lost"
                                >
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    move(p.id, "lead");
                                    toast.success(`"${p.name}" reopened to Lead stage`);
                                  }}
                                  className="opacity-0 group-hover:opacity-100 transition text-muted-foreground hover:text-emerald-500"
                                  title="Reopen project to Lead"
                                  aria-label="Reopen project to Lead"
                                >
      ...
      {quoteProject && (
        <ProjectQuoteDialog
          open={!!quoteProject}
          onOpenChange={(v) => !v && setQuoteProject(null)}
          projectId={quoteProject.id}
          projectName={quoteProject.name}
          projectType={quoteProject.project_type}
          currentNotes={quoteProject.notes}
          currentValue={quoteProject.opportunity_value}
          onQuoteApplied={() => {
            qc.invalidateQueries({ queryKey: ["projects"] });
          }}
        />
      ...
      <ProjectNotesDialog
        ...
        clientName={notesProject ? clientName.get(notesProject.client_id) : undefined}
...
function NewProjectButton({ clients }: { clients: { id: string; name: string }[] }) {
  const qc = useQueryClient();
  const { user } = useAuth();
  const { projectTypes, addType } = useProjectTypes();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    if (!clientId) {
      const msg = "Please select or add a client for this project.";
      setFormError(msg);
      toast.error(msg);
      return;
    }
    ...
    const initialImplStage = status === "work_in_progress" ? "kickoff" : status === "delivered" ? "done" : null;
    const { error } = await supabase.from("projects").insert({
      client_id: clientId,
      name: name.trim(),
      status,
      impl_stage: initialImplStage,
    ...
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) reset(); else if (clients.length === 0) setAddingClient(true); }}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={submit} className="space-y-4">
          <FormErrorAlert error={formError} onDismiss={() => setFormError(null)} title="Failed to add project" />
          <div>
            ...
            {addingClient ? (
              <div className="mt-1.5 space-y-2">
                <FormErrorAlert error={clientError} onDismiss={() => setClientError(null)} title="Failed to add client" />
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setAddingProjectType((v) => !v)}
                className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground hover:text-foreground"
              >
                {addingProjectType ? "Pick existing" : "+ New project type"}
              </button>
            </div>
            {addingProjectType ? (
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      ...
                      if (newProjectTypeName.trim()) {
                        const added = addType(newProjectTypeName.trim());
                        setProjectType(added);
                        setNewProjectTypeName("");
                        setAddingProjectType(false);
                        toast.success(`Project type "${added}" added`);
                      }
                    }
                  }}
                />
    ...
    <div className="mb-8 border border-border rounded-lg bg-card overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-border">
        <div className="px-5 py-5">
          <div className="font-mono text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground">Open pipeline</div>
          <div className="mt-1.5 font-display text-3xl md:text-4xl font-extrabold leading-none tracking-tight text-foreground tabular-nums">
            {formatZAR(totalOpen)}
          </div>
          <div className="mt-1 text-xs text-foreground/70 font-normal">
            Lead → Review, unweighted
          </div>
        </div>
        <div className="px-5 py-5">
          <div className="font-mono text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground">Weighted forecast</div>
          <div className="mt-1.5 font-display text-3xl md:text-4xl font-extrabold leading-none tracking-tight text-foreground tabular-nums">
            {formatZAR(weighted)}
          </div>
          <div className="mt-1 text-xs text-foreground/70 font-normal">
        ...
        <div className="px-5 py-5">
          <div className="font-mono text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground">
            Delivered · {new Date().getFullYear()}
          </div>
          <div className="mt-1.5 font-display text-3xl md:text-4xl font-extrabold leading-none tracking-tight tabular-nums text-emerald-700 dark:text-emerald-400">
            {formatZAR(wonYtd)}
          </div>
          <div className="mt-1 text-xs text-foreground/70 font-normal">Year to date</div>
        </div>
      </div>
