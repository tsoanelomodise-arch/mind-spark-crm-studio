...
function PromptDetail() {
  ...
  const exitEdit = () => {
    setEditing(false);
    if (search.edit) router.navigate({ to: "/prompts/$id", params: { id }, search: {} });
  };
  ...
  const duplicate = async () => {
    ...
    qc.invalidateQueries({ queryKey: ["prompts"] });
    router.navigate({ to: "/prompts/$id", params: { id: data.id }, search: { edit: true } });
  };
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      <button
        onClick={() => router.navigate({ to: "/prompts" })}
        className="flex items-center gap-1.5 font-mono text-xs uppercase tracking-widest text-muted-foreground hover:text-foreground mb-6 cursor-pointer"
      >
        <ArrowLeft className="h-3.5 w-3.5" /> Wiki / Prompts Library
      </button>
