...
function NewPrompt() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    router.navigate({ to: "/prompts/$id", params: { id: data.id } });
  };
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      ...
      <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Wiki / Prompts Library · New Entry</p>
      <h1 className="mt-2 font-display text-4xl font-semibold">Add a prompt to the shelf.</h1>

      <form onSubmit={save} className="mt-8 space-y-6">
        <FormErrorAlert error={error} onDismiss={() => setError(null)} title="Failed to save prompt" />
        <div>
