...
function safeFormat(dateStr: string | null | undefined, fmt = "PP"): string {
  if (!dateStr) return "—";
  try {
...
function ClientDetail() {
  ...
  if (isLoading) {
    return <div className="mx-auto max-w-6xl px-6 py-10 text-sm text-muted-foreground">Loading client profile…</div>;
  }
  ...
  if (isError || !client) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-16 text-center">
        ...
        <h2 className="font-display text-2xl font-semibold">Client not found</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          This client record may have been deleted or the link is invalid.
        </p>
        <Button onClick={() => router.navigate({ to: "/clients" })} className="mt-6 gap-2">
          <ArrowLeft className="h-4 w-4" /> Back to Clients roster
        </Button>
      </div>
    );
  }
      ...
      <div className="pb-8 border-b border-border">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
            ...
            <DeleteClientButton clientId={clientId} clientName={client.name} redirectOnDelete={true} variant="ghost" size="sm" />
          </div>
        </div>
      </div>
      ...
      <aside className="space-y-4 font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
        <div>
          ...
          <div className="mt-1 text-foreground normal-case font-sans">
            {safeFormat(client.created_at, "PP")}
          </div>
        </div>
        <div>
          <div>Updated</div>
          <div className="mt-1 text-foreground normal-case font-sans">
            {safeFormatDistance(client.updated_at)}
          </div>
        </div>
      </aside>
    </div>
...
function ProjectsPane({ clientId }: { clientId: string }) {
  ...
  const [quoteProject, setQuoteProject] = useState<any>(null);
  ...
  const updateStage = async (projectId: string, value: PipelineStage) => {
    ...
    const updates: { status: PipelineStage; delivered_at?: string | null; impl_stage?: string | null } = { status: value };
    if (value === "work_in_progress" && !(current as any)?.impl_stage) updates.impl_stage = "kickoff";
    if (value === "delivered" && !(current as any)?.delivered_at) {
      updates.delivered_at = new Date().toISOString();
      if (!(current as any)?.impl_stage) updates.impl_stage = "done";
    }
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button
                      ...
                      className="h-7 text-[10px] gap-1 font-mono uppercase tracking-widest px-2"
                      title="AI Price Quote & Effort Estimator"
                    >
...
function CredentialsPane({ clientId }: { clientId: string }) {
  ...
  const add = async (e: React.FormEvent) => {
    e.preventDefault();
    const computedLabel = form.system.trim() || client?.name || "Client Login";
    const { data, error } = await supabase
      .from("credentials")
      .insert({
        client_id: clientId,
...
function CredentialRow({
  ...
  const save = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const computedLabel = edit.system.trim() || clientName || cred.label || "Client Login";
    const { error } = await supabase
      .from("credentials")
      .update({
        label: computedLabel,
    ...
    <div className="border border-border rounded-lg bg-card p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {cred.system && cred.system.trim().toLowerCase() !== cred.label.trim().toLowerCase() && (
              <Badge variant="secondary" className="font-mono text-[10px] font-normal">{cred.system}</Badge>
            )}
          </div>
          {cred.url && (
            <a href={formatExternalUrl(cred.url)} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 font-mono text-xs text-muted-foreground hover:text-foreground break-all">
      ...
      {cred.last_rotated_at && (
        <p className="mt-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
          Rotated {safeFormatDistance(cred.last_rotated_at)}
        </p>
      )}
    </div>
  );
              ...
              <li key={c.id} className="relative">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                      {ch.label} · {safeFormat(c.occurred_at, "PPp")}
                    </p>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  ...
                  {c.follow_up_at && (
                    <span className="inline-flex items-center gap-1 text-foreground">
                      <CalendarClock className="h-3 w-3" /> Follow up {safeFormat(c.follow_up_at, "PP")}
                    </span>
                  )}
                </div>
              </li>
